...
"""Incarcare Model"""
def load_satlas_s1_backbone(checkpoint_path: str):
    """Model Sentinel-1 pretrained"""
    try:
        model = weights_manager.get_pretrained_model(
            model_identifier=S1_MODEL_ID,
            checkpoint_path=checkpoint_path
        )
        model.eval() 
        return model
    except Exception as e:
        print(f"ERROR: Failed to load Satlas model. Ensure .pth file is at {checkpoint_path}")
        print(f"Details: {e}")
        return None

"""Extragere Features + Transformare pt inferare"""
def integrate_and_infer(raw_sar_data: np.ndarray) -> Dict[tuple, np.ndarray]: #raw_sar_data este un numpy array care contine datele "raw ale imaginii" -> asta s-ar obtine cu din partea lui Ionut+Dana (alt fisier .py in mod normal)
